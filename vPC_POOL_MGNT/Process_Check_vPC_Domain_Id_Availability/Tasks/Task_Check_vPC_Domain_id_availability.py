...
if not context.get('searchedVpcId'):
  MSA_API.task_error('Please enter an vPC Id to search', context)
  
searchedVpcId=context['searchedVpcId']

# Check if given vPC Id is not starting with 0 (eg : 01)
if searchedVpcId.startswith('0'):
  MSA_API.task_error('vPC Id '+searchedVpcId+" not valid, please retry", context)
...
if int(context['SelectedVpcRangeStart']) > int(searchedVpcId) or int(searchedVpcId) > int(context['SelectedVpcRangeEnd']):
  MSA_API.task_error('vPC Id '+searchedVpcId+" not on the available range ("+context['SelectedVpcRangeStart']+" - "+context['SelectedVpcRangeEnd']+")", context)
...
for vpcIdInUse in context['vpcsInUse']:
  if (searchedVpcId == vpcIdInUse['vpcId']) and (str(vpcIdInUse['assignment_information']) == 'From vPC Pool '+context['SelectedVpcRangeStart']+' - '+context['SelectedVpcRangeEnd']+''):
    ...
    break
if not freeVpcId:
  MSA_API.task_error('vPC Id '+searchedVpcId+' is already in use in Pool range '+context['SelectedVpcRangeStart']+' - '+context['SelectedVpcRangeEnd']+'', context)

MSA_API.task_success('The vPC Id '+searchedVpcId+' is available in Pool range '+context['SelectedVpcRangeStart']+' - '+context['SelectedVpcRangeEnd']+'', context)
